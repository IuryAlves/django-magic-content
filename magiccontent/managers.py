...
class AreaManager(SiteManager):
    ...
    def get_or_create(self, name):
        return super(AreaManager, self).get_query_set().get_or_create(name=name,
                                                                      site=Site.objects.get_current())
    def get(self, name):
        return super(AreaManager, self).get_query_set().get(name=name,
                                                            site=Site.objects.get_current())
    def actives(self):
...
class WidgetManager(SiteManager):
    ...
    def list_content_from_type(self, widget):
        ...
        return content_type_class.site_objects.filter(widget=widget, is_active=True)
    ...
    def create(self, widget_type, style_template, name=''):
        if not name:
            ...
            name = "%s_%s_%s" % (widget_type, current_time.strftime("%d%H%M%S%f"), randint(128, 1024))

        new_widget, _ = super(WidgetManager, self).get_query_set().get_or_create(name=name,
                           widget_type=widget_type, style_template=style_template)
        new_widget.save()
